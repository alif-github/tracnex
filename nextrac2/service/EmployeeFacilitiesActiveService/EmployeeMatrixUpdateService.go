...
type employeeMatrixUpdateService struct {
	FileName string
	service.AbstractService
}

var EmployeeMatrixUpdateService = employeeMatrixUpdateService{FileName: "EmployeeMatrixUpdateService.go"}

func (input employeeMatrixUpdateService) UpdateEmployeeMatrix(request *http.Request, context *applicationModel.ContextModel) (output out.Payload, header map[string]string, err errorModel.ErrorModel) {
	...
	_, err = input.InsertServiceWithAudit("UpdateEmployeeMatrix", matrixBody, context, input.doUpdate, func(interface{}, applicationModel.ContextModel) {})
	if err.Error != nil {
		return
	}
...
func (input employeeMatrixUpdateService) doUpdate(tx *sql.Tx, body interface{}, context *applicationModel.ContextModel, now time.Time) (_ interface{}, auditData []repository.AuditSystemModel, err errorModel.ErrorModel) {
	...
	for i:=0; i < len(matrixBody.AllowanceList); i++{
		...
		idMatrix, err2 := dao.EmployeeFacilitiesActiveDAO.CekEmployeeMatrixForUpdate(tx, allowanceRepo, "allowance_id", matrixBody.AllowanceList[i].ID)
		if err2.Error != nil {
			err = err2
			return
		}
		...
		if idMatrix == 0 {
			...
			allowanceRepo.ID.Int64 = idMatrix
            _ = dao.EmployeeFacilitiesActiveDAO.UpdateEmployeeMatrix(tx, allowanceRepo)
		}
	}
	...
	for i:=0; i < len(matrixBody.BenefitList); i++{
		...
		idMatrix, err2 = dao.EmployeeFacilitiesActiveDAO.CekEmployeeMatrixForUpdate(tx, benefitRepo, "benefit_id", matrixBody.BenefitList[i].ID)
		...
		if idMatrix == 0 {
			...
			benefitRepo.ID.Int64 = idMatrix
			_ = dao.EmployeeFacilitiesActiveDAO.UpdateEmployeeMatrix(tx, benefitRepo)
		}
	}
...
func (input employeeMatrixUpdateService) getEmpMatrixRepository(matrix in.EmployeeMatrixRequest, typ string, index int64, authAccessToken model2.AuthAccessTokenModel, now time.Time) repository.EmployeeFacilitiesActiveModel {
	value := ""
	active := false
...
func (input employeeMatrixUpdateService) validasiValue(value string) (err errorModel.ErrorModel){
